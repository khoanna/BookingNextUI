...
import { useGetOtpRegister } from '../services/authenticate/useGetOTPRegister';
import { useGetOTPForget } from '../services/authenticate/useGetOTPForget';
import { useChangePass } from '../services/authenticate/useChangePass'
import { useLogin } from '../services/authenticate/useLogin';
import { useRegister } from '../services/authenticate/useRegister';
import { useGoogleLogin } from '../services/authenticate/useGoogleLogin';
...
export default function Login() {
    ...
    const { getOtpForget } = useGetOTPForget();
    const { getOtpRegister } = useGetOtpRegister();

    const { googleLogin } = useGoogleLogin();
    const { registerLoading, register } = useRegister();
    const { changePass, changePassLoading } = useChangePass();
    ...
    const handleGetOtpForget = async () => {
        if (!isValidEmailCommon(forgetEmail)) {
            showToast('error', 'Vui lòng nhập mail hợp lệ');
            return;
        }
        setOtpCountdown(60);
        await getOtpForget({ email: forgetEmail });
        showToast('success', 'Kiểm tra mail để nhận OTP');
    }
    ...
    const handleChangePass = async () => {
        ...
        if (!forgetEmail || !forgetOTP || !newPass || !confirmNewPass) {
            showToast('error', 'Vui lòng nhập đủ thông tin để tiếp tục')
            return;
        }
        const respone = await changePass({ email: forgetEmail, otp: forgetOTP, newPassword: newPass, confirmPassword: confirmNewPass })
        const data = await respone.json();
        showToast(respone.status == 200 ? 'success' : 'error', data.data.body);
        setForgetEmail('');
        setForgetOTP('');
        setNewPass('');
        setConfirmNewPass('');
    ...
    const handleLogin = async () => {
        ...
        const respone = await login({ email: loginMail, password: loginPass });
        const data = await respone.json();
            ...
            <div className="md:w-1/2 w-full flex flex-col items-center justify-center">
                {status == 'login' && (
                    <div className="md:w-96 w-80 flex flex-col items-center justify-center">
                        ...
                        <div className="w-full flex flex-col md:flex-row gap-2 items-center justify-between mt-8 px-2 text-gray-500">
                ...
                {status == 'forgot' && (
                    <div className="md:w-96 w-80 flex flex-col items-center justify-center">
                        ...
                        <div className="flex items-center w-full bg-transparent border-2 border-gray-300 focus-within:border-gray-400 h-12 rounded-full overflow-hidden pl-6 gap-2 mt-5">
                            ...
                            <input value={forgetEmail} onChange={e => setForgetEmail(e.target.value)} type="email" placeholder="Email" className="bg-transparent text-gray-500 placeholder-gray-500/80 outline-none text-sm w-full h-full" />
                        </div>

                        <div className="flex items-center mt-6 w-full bg-transparent border-2 border-gray-300 focus-within:border-gray-400 h-12 rounded-full overflow-hidden pl-6 gap-2">
                            <input value={forgetOTP} onChange={e => setForgetOTP(e.target.value)} placeholder="Mã OTP" className="bg-transparent text-gray-500 placeholder-gray-500/80 outline-none text-sm w-full h-full" />
                            <div onClick={handleGetOtpForget} className='text-xs text-center w-4/12 mr-4 cursor-pointer text-gray-500 hover:text-black'>
                        ...
                        <div className="flex items-center mt-6 w-full bg-transparent border-2 border-gray-300 focus-within:border-gray-400 h-12 rounded-full overflow-hidden pl-6 gap-2">
                            ...
                            <input value={newPass} onChange={e => setNewPass(e.target.value)} type="password" placeholder="Mật khẩu mới" className="bg-transparent text-gray-500 placeholder-gray-500/80 outline-none text-sm w-full h-full" />
                        </div>
                        ...
                        <div className="flex items-center mt-6 w-full bg-transparent border-2 border-gray-300 focus-within:border-gray-400 h-12 rounded-full overflow-hidden pl-6 gap-2">
                            ...
                            <input value={confirmNewPass} onChange={e => setConfirmNewPass(e.target.value)} type="password" placeholder="Xác nhận mật khẩu mới" className="bg-transparent text-gray-500 placeholder-gray-500/80 outline-none text-sm w-full h-full" />
                        </div>

                        <div onClick={handleChangePass} className="flex items-center justify-center cursor-pointer mt-8 w-full h-10 rounded-full text-white bg-indigo-500 hover:opacity-90 transition-opacity">
                            {changePassLoading ?
                                (<svg
                                    className="animate-spin h-4 w-4 text-white"
                                    xmlns="http://www.w3.org/2000/svg"
                ...
                {status == 'register' && (
                    <div className="md:w-96 w-80 flex flex-col items-center justify-center">
                        ...
                        <div className="flex items-center mt-6 w-full bg-transparent border-2 border-gray-300 focus-within:border-gray-400 h-12 rounded-full overflow-hidden pl-6 gap-2">
                            ...
                            <button onClick={handleGetOtpRegister} disabled={OtpCountdown} className='text-xs text-center w-4/12 mr-4 cursor-pointer text-gray-500 hover:text-black'>
                                {OtpCountdown ? OtpCountdown : "Nhận mã"}
                            </button>
                        </div>
